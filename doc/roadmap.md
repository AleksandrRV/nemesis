...
> **Статус планирования на 18.09.2026:** версия проекта зафиксирована как **0.2.0**. Ниже этап v0.3.0
...
### Шаг 1. Зафиксировать контракт v0.3.0 и собрать источники данных
...
**Не хватает сканов оригинала:**

* лицевых и оборотных сторон 10 карт Действий каждого из шести персонажей;
* памятки игрока с вариантами микрохода и стороной «Пас»;
* планшетов персонажей, если по ним нужно проверить слоты руки, лимит карт и визуальные обозначения.
...
* всех карт Предметов Красной, Жёлтой и Зелёной колод, включая обороты;
* стартовых карт оружия шести персонажей;
* карт Заражения и Тяжёлых Травм, если они входят в реализацию соответствующих действий v0.3.0;
* карточек компонентов и символов, если они будут отображаться в интерфейсе.
...
* новых сканов не требуется для серверной логики, если материалы из шага 2 доступны;
* для визуальной проверки стоимости и пиктограмм нужны сканы карт Действий и памятки игрока из шага 1.
...
* сканы не нужны для FSM;
* для проверки порядка и визуальных подсказок желательно получить скан памятки игрока и жетона Первого Игрока.
...
* сканы всех карт Предметов из шага 2;
* сканы лицевой стороны тайлов/жетонов комнат с цветом и счётчиком Предметов;
* сканы планшета персонажа со слотами руки и зоной скрытого инвентаря.

### Шаг 6. Реализовать действия комнат и определить зависимости последующих этапов
...
* Для Операционной явно зафиксировать зависимость от v0.6: либо реализовать минимальный интерфейс-заглушку,
  либо перенести полное действие на этап Красного Сканера.
* Отдельно принять решение по Радиорубке, Спасательным отсекам, Лаборатории, Улью и Складу, потому что roadmap
  называет 11 комнат, но перечисляет только пять.

**Не хватает сканов оригинала:**

* сканы лицевых сторон всех 11 тайлов комнат категории «1», прежде всего Оружейной, Лазарета, Генератора,
  Комнаты пожарной безопасности и Операционной;
* сканы жетонов/треков самоуничтожения, боезапаса и тяжёлых травм;
* сканы карт или памяток, описывающих варианты действий комнат.

### Шаг 7. Подключить UI Фазы Игроков, руки и приватные решения

* Добавить нижнюю панель руки с читаемыми картами, стоимостью и состоянием выбора.
* Добавить выбор карт для оплаты и кнопки двух вариантов микрохода плюс кнопку паса.
* Добавить отображение сброса, лимита руки, активного игрока и оставшихся действий.
* Добавить скрытый инвентарь, два слота руки и обработку замены тяжёлого предмета.
* Добавить модальное окно выбора одной из двух карт Поиска и выбора колоды в белом отсеке.
* Добавить контекстные кнопки действий комнаты и диалоги выбора целей.
* Переразложить нижние элементы вместе с журналом и `RoomInspector`, чтобы панели не перекрывали друг друга.
* Использовать только `SanitizedGameState`: содержимое чужой руки, инвентаря и промежуточного выбора не должно
  попадать в общий UI.

**Не хватает сканов оригинала:**

* сканы планшетов персонажей, слотов руки и памятки игрока;
* сканы карт Действий и Предметов для проверки читабельности UI;
* сканы тайлов комнат и символов действий для точного отображения комнатных консолей.
...
* для автоматических тестов сканы не нужны;
* для финальной сверки 1:1 всё ещё нужны комплекты из шагов 1, 2, 5 и 6 — без них можно проверить правила,
  но нельзя подтвердить состав, пиктограммы и визуальные тексты оригинальных компонентов.
