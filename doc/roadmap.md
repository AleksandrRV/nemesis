...
1. **Автосохранение сессии (Mobile Persistence):**
   * В 0.5.0 уже: сериализация `GameState` в `sessionStorage` (`nemesis-session`, схема 19) после каждого действия, восстановление при входе. На этапе 0.9.0 — расширить до `persist`/`localStorage` + PWA-офлайн при необходимости.
...
### Задачи:
1. **Бэкенд `packages/server` (цель, текущая сборка — `LocalInMemoryTransport` без сервера):**
