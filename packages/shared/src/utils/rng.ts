...
/**
 * Сидированная случайность партии (AGENTS.md §4.3).
 *
 ...
 *
 * Счётчик обращений лежит в состоянии партии (`meta.rngDraws`), а сам генератор
 * там не хранится: состояние остаётся JSON-сериализуемым и не зависит от
 * библиотеки. Так сохранение продолжает ту же последовательность, а не начинает
 * её заново (план исправлений, Э2-4).
 */
...
/**
 * Нулевые счётчики обращений по всем потокам: стартовое значение `meta.rngDraws`.
 *
 * Объект перечисляет потоки явно: добавление нового потока в `RNG_STREAMS`
 * не соберётся без правки этого места, и счётчик не потеряется молча.
 */
export function createRngDraws(): Record<RngStream, number> {
  return { layout: 0, noise: 0, bag: 0, cards: 0, combat: 0 };
}
 ...
 * обращений лежит в `meta.rngDraws`, а сам генератор там не хранится — состояние
