...
| **Презентация Фазы Событий**<br>`events/EventPhaseModal.tsx`<br>`events/eventPhaseModalModel.ts`<br>`events/EventPhaseBanner.tsx`<br>`events/eventPhaseBannerModel.ts` | Кинематографичный оверлей «ФАЗА СОБЫТИЙ: РАУНД N» (Шаг 9): шесть шагов в порядке движка (счётчики, атаки, огонь, карта Событий, текстовый эффект, Улей) с записями Журнала, навигацией НАЗАД/ДАЛЕЕ и клавиатурой; открывается автоматически один раз на каждую Фазу Событий, баннер-сводка атак на время показа скрывается. | Читает `view.gameLog` и сообщает карте подсвечиваемые отсеки через `onStepChange`; партию не изменяет. |
